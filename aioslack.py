...
class Client(object):
    PRODUCER_DELAY = 0.5 # debounce delay in seconds to avoid overloading the websocket connection
    ...
    async def find_channel(self, channel_name):
        if self.channels:
            ...
            self.channels = await self.get("channels.list",
    ...
    async def find_user(self, user_name):
        if self.users:
            ...
            self.users = await self.get("users.list",
    ...
    async def handle_http_response(self, resp, req_name, callback=None):
        status = resp.status
        if status != 200:
            logging.error('{} request was not successful [Code: {}]'.format(req_name, status))
